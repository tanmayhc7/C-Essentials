...
void *functionCount1(void *vargs)
{
   for(;;)
   {
...
void *functionCount2(void *vargs)
{
    for(;;)
    {
       pthread_mutex_lock( &condition_mutex );
...
The only thing that has been ensures is that functionCount2 will increment the count between the values COUNT_HALT1 and COUNT_HALT2. Everything else is random.
